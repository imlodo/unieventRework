const BASE_URL = 'http://localhost:7071';
export const ADD_CONTENT_BOOKED = `${BASE_URL}/api/content/addContentBooked`;
export const ADD_CONTENT = `${BASE_URL}/api/content/addContent`;
export const ADD_COUPON = `${BASE_URL}/api/content/addCoupon`;
export const ADD_LIKE_BY_TYPE = `${BASE_URL}/api/content/addLikeByType`;
export const ADD_TICKET_REVIEW = `${BASE_URL}/api/ticket/addTicketReview`;
export const ADD_SUPPORT_TICKET_REPLY = `${BASE_URL}/api/support/addSupportTicketReply`;
export const ADD_CHAT_REPLY = `${BASE_URL}/api/user/addChatReply`;
export const ADD_DISCUSSION = `${BASE_URL}/api/content/addDiscussion`;
export const ADD_USER_CARD = `${BASE_URL}/api/user/addUserCard`;
export const ADD_USER_ADDRESS = `${BASE_URL}/api/user/addUserAddress`;
export const CHECK_CONTENT_IS_BOOKED_BY_CURRENT_USER = `${BASE_URL}/api/content/checkContentIsBookedByCurrentUser`;
export const CHECK_CONTENT_IS_LIKED_BY_CURRENT_USER = `${BASE_URL}/api/content/checkContentIsLikedByCurrentUser`;
export const CHECK_IS_FOLLOWED = `${BASE_URL}/api/user/checkIsFollowedByCurrentUser`;
export const CHECK_IS_FOLLOW_REQUEST_BY_CURRENT_USER = `${BASE_URL}/api/user/checkIsFollowRequestByCurrentUser`;
export const CREATE_NEW_SUPPORT_TICKET = `${BASE_URL}/api/support/createNewTicket`;
export const CREATE_NEW_USER = `${BASE_URL}/api/user/createNewUser`;
export const DOWNLOAD_PERSONAL_DATA = `${BASE_URL}/api/user/downloadPersonalData`;
export const DELETE_ACCOUNT = `${BASE_URL}/api/user/DeleteAccount`;
export const DELETE_CONTENT = `${BASE_URL}/api/content/deleteContent`;
export const DELETE_COUPON = `${BASE_URL}/api/content/deleteCoupon`;
export const EDIT_USER = `${BASE_URL}/api/user/EditUser`;
export const FOLLOW_USER = `${BASE_URL}/api/user/followUser`;
export const FOLLOW_USER_REQUEST = `${BASE_URL}/api/user/followUserRequest`;
export const UNFOLLOW_USER = `${BASE_URL}/api/user/unfollowUser`;
export const UN_REQUEST_FOLLOW_USER = `${BASE_URL}/api/user/unReqFollowUser`;
export const GENERATE_TOKEN = `${BASE_URL}/api/token/generateToken`;
export const GENERATE_TOKEN_FOR_BUY = `${BASE_URL}/api/token/generateTokenForBuy`;
export const GET_BUY_TOKEN_DETAIL = `${BASE_URL}/api/token/getBuyTokenDetail`;
export const GET_COUPON_DISCOUNT_PERCENTAGE = `${BASE_URL}/api/content/getCouponDiscountPercentage`;
export const GET_USER = `${BASE_URL}/api/user/getUser`;
export const GET_USER_FOLLOWED_BY_CURRENT_USER = `${BASE_URL}/api/user/getUserFollowedByCurrentUser`;
export const GET_USER_CREDIT_CARDS = `${BASE_URL}/api/user/getUserCreditCards`;
export const GET_USER_ADDRESS = `${BASE_URL}/api/user/getUserAddress`;
export const GET_VERIFY_ACCOUNT_LIST = `${BASE_URL}/api/user/getVerifyAccountList`;
export const GET_CHAT_LIST = `${BASE_URL}/api/user/getChatList`;
export const GET_CONTENT_DISCUSSIONS = `${BASE_URL}/api/content/getContentDiscussions`;
export const GET_CHAT_MESSAGE_LIST = `${BASE_URL}/api/user/getChatMessageList`;
export const GET_EVENT_BY_NAME = `${BASE_URL}/api/content/getEventByName`;
export const GET_CONTENTS_BY_CURRENT_USER = `${BASE_URL}/api/content/getContentsByCurrentUser`;
export const GET_COUPONS_FOR_EVENT = `${BASE_URL}/api/content/getCouponsForEvent`;
export const GET_MORE_CONTENT = `${BASE_URL}/api/content/getMoreContent`;
export const GET_MORE_CONTENT_BASED_ON_CURRENT_USER = `${BASE_URL}/api/content/getMoreContentBasedOnCurrentUser`;
export const GET_USER_PROFILE_INFO = `${BASE_URL}/api/user/getUserProfileInfo`;
export const GET_USER_SETTINGS = `${BASE_URL}/api/user/getUserSettings`;
export const GET_PROFILE_USER_SETTINGS = `${BASE_URL}/api/user/getProfileUserSettings`;
export const GET_TICKET_LIST = `${BASE_URL}/api/ticket/getTicketList`;
export const GET_SINGLE_CONTENT = `${BASE_URL}/api/content/getSingleContent`;
export const GET_RELATED_EVENTS = `${BASE_URL}/api/content/getRelatedEvents`;
export const GET_SUPPORT_TICKET_LIST = `${BASE_URL}/api/support/getSupportTicketList`;
export const GET_SUPPORT_TICKET_DISCUSSION_LIST = `${BASE_URL}/api/support/getSupportTicketDiscussionList`;
export const GET_TICKET_REVIEWS = `${BASE_URL}/api/ticket/getTicketReviews`;
export const GET_TICKET_DETAIL = `${BASE_URL}/api/ticket/getTicketDetail`;
export const GET_REQUEST_PERSONAL_DATA_STATUS = `${BASE_URL}/api/user/getRequestPersonalDataStatus`;
export const GET_VERIFY_ACCOUNT_STATUS = `${BASE_URL}/api/user/getVerifyAccountStatus`;
export const PURCHASE_TICKET = `${BASE_URL}/api/ticket/purchase`;
export const REQUEST_PERSONAL_DATA = `${BASE_URL}/api/user/requestPersonalData`;
export const SAVE_USER_SETTINGS = `${BASE_URL}/api/user/saveUserSettings`;
export const SEND_NEW_PASSWORD = `${BASE_URL}/api/user/sendNewPassword`;
export const SEND_CONFIRMATION_EMAIL = `${BASE_URL}/api/user/sendConfirmationEmail`;
export const SEARCH_USER = `${BASE_URL}/api/user/searchUser`;
export const UPLOAD_FILE = `${BASE_URL}/api/file/uploadFile`;
export const UPLOAD_FILE_VIDEO_PREVIEW = `${BASE_URL}/api/file/generateVideoPreview`;
export const UPDATE_CONTENT_PRIVACY = `${BASE_URL}/api/content/updateContentPrivacy`;
export const UPDATE_COUPON = `${BASE_URL}/api/content/updateCoupon`;
export const UPDATE_VERIFY_ACCOUNT = `${BASE_URL}/api/user/updateVerifyAccount`;
export const DELETE_FILE = `${BASE_URL}/api/file/deleteFile`;
export const DOWNLOAD_FILE = `${BASE_URL}/api/file/downloadFile`;
export const VERIFY_TOKEN = `${BASE_URL}/api/token/verifyToken`;
export const VERIFY_ACCOUNT = `${BASE_URL}/api/user/verifyAccount`;
export const NEGOTIATE = `${BASE_URL}/api/Negotiate`;