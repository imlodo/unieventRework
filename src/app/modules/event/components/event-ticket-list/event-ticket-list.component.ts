...
export class EventTicketListComponent implements AfterViewInit {
    ...
    if (this.ebtc) {
      ...
      this.eventData.t_map_list.forEach(t_maps => {
        t_maps.t_object_maps.forEach(el => {
          if (!seatType[getTicketNameByType(el.t_type)] && el.is_acquistabile) {
            ...
            const formConst = this.ebtc.formRows.controls.filter(form => form.get("ticket_title").value === getTicketNameByType(el.t_type));
  ...
  buyBestTickets(eventBuyTicketRequestList: Array<EventBuyTicketRequest>) {
  ...
  getTicketListByBestTicketType(eventBuyTicketRequestList: Array<EventBuyTicketRequest>) {
    ...
    eventBuyTicketRequestList.forEach(el => {
      let objectMapFiltered: ObjectMap[] = null;
      this.eventData.t_map_list.forEach(map => {
        objectMapFiltered = map.t_object_maps.filter(obj => obj.n_id === el.n_object_id);
        if (objectMapFiltered.length > 0)
          return;
      });
    ...
    let eventDataCopy: Event = {
      ...
      t_map_list: [],
      t_company: this.eventData.t_company,
